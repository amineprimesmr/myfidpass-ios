...
//    2. Le fond détecté est uniforme : >= 65 % des pixels de bord sont proches de la couleur
//       estimée via les 4 coins. Évite de toucher aux photos ou fonds complexes.
...
enum FlyerLogoBackgroundPrepared {
    ...
    /// Retire les pixels de fond si le fond est uniforme (couleur estimée sur les 4 coins,
    /// vérifiée sur tous les pixels de bord). Si le fond n'est pas uniforme → nil (pas de modification).
    private static func removeUniformBackgroundFromEdges(from image: UIImage) -> UIImage? {
        ...
        return data.withUnsafeMutableBytes { raw -> UIImage? in
            ...
            for (cx, cy) in corners {
                let o = (cy * w + cx) * bpp
                sumR += Int(base[o]); sumG += Int(base[o + 1]); sumB += Int(base[o + 2])
            }
            let bgR = sumR / 4, bgG = sumG / 4, bgB = sumB / 4

            // ── 2. Uniformité du fond sur tous les pixels de bord ────────────────────
            let tol = 38  // tolérance par canal (absorbe artefacts JPEG, anti-aliasing)
            ...
            func checkEdge(_ x: Int, _ y: Int) { total += 1; if isBg(y * w + x) { matched += 1 } }
            for x in 0..<w { checkEdge(x, 0); checkEdge(x, h - 1) }
            for y in 1..<(h - 1) { checkEdge(0, y); checkEdge(w - 1, y) }

            // Fond non uniforme (photo, scène complexe) → ne pas toucher
            guard Double(matched) / Double(max(1, total)) >= 0.65 else { return nil }
